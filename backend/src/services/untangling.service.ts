...
export const untanglingService = {
  async complete(input: z.infer<typeof UntanglingSchema>, userId: string) {
    const op = await productionOrderRepository.findById(input.po_id);
    if (!op) throw new NotFoundError('Ordem de produção');
    await db.transaction(async (client) => {
      await client.query(
        `INSERT INTO po_untangling (op_id, employee_id, notes, processed_at)
         VALUES ($1,$2,$3,$4)`,
        [input.po_id, input.employee_id, input.notes ?? null, input.timestamp]
      );
