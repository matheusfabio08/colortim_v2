...
export default function Dashboard() {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return <span className={`px-3 py-1 rounded-full text-xs font-semibold ${statusInfo.color}`}>{statusInfo.label}</span>;
  };
    ...
    <Layout>
      <div className="h-full flex flex-col overflow-hidden">
        ...
        <div className="flex-shrink-0 grid grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
          <KPICard title="OPs em Andamento" value={kpis?.active_ops || 0} icon={Package} color="blue" />
          <KPICard title="OPs Atrasadas" value={kpis?.overdue_ops || 0} icon={AlertCircle} color="red" />
          <KPICard title="Concluídas Hoje" value={kpis?.completed_today || 0} icon={CheckCircle} color="green" />
          <KPICard title="Taxa de Produtividade" value={`${kpis?.productivity_rate || 0}%`} icon={TrendingUp} color="yellow" />
        </div>
          ...
          <div className="flex-shrink-0 p-3 border-b border-gray-200">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="Buscar OP, cliente ou cor..." value={searchTerm}
                ...
                {filteredOPs && filteredOPs.length > 0 ? (
                  filteredOPs.map((op) => (
                    <tr key={op.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-3 py-2"><span className="font-semibold text-blue-600">{op.op_number}</span></td>
                      <td className="px-3 py-2 text-gray-900">{(op as any).material || '-'}</td>
                      <td className="px-3 py-2 text-gray-900">{op.client}</td>
                      <td className="px-3 py-2 text-gray-900">{op.color}</td>
                      <td className="px-3 py-2">{getStatusBadge(op.status)}</td>
                      <td className="px-3 py-2 text-gray-600">{format(new Date(op.expected_date), "dd/MM/yyyy", { locale: ptBR })}</td>
                      <td className="px-3 py-2">
                        <Link to={`/op/${op.id}`} className="text-blue-600 hover:text-blue-800 font-medium">Ver →</Link>
                      </td>
                    </tr>
                  ))
                ) : (
                  <tr><td colSpan={7} className="px-3 py-6 text-center text-gray-500">Nenhuma ordem de produção encontrada</td></tr>
                )}
              </tbody>
            </table>
          </div>
